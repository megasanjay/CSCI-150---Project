...
Menu *menu = new Menu(); // pointer menu is now pointing to a new menu
Menu *menuhead; // pointer head to be used to point to the first menu

void menuPopulate() {
    string Word;
    int f = 0;
    int p =0;
    infile.open("/Users/martinregalado/Desktop/Menu/Menu.txt");
    outfile.open("/Users/martinregalado/Desktop/Menu/Menu_output.txt");
    infile >> Word;
    menuhead = menu;
    while(!infile.eof()){
        ...
        if (Word == "Item:") { // if it reads "Item" it means that the next word is going to be a menu item
            ...
            if (Word != "Price:"){ // if the word is not price then it means there is more to the word so it adds more words to the previous Item array
                while(Word != "Price:") { // continues to add words the the previous array cell till the word is finished
        ...
        if (Word == "Price:") { // when the word is price it add the price to the [p element of the array]
            infile >> menu->Price[p]; //
            p++;// moves to the next cell of the price array
            infile >> Word; // reads in another string
        }
        if (Word == "Done") { // if the keyword "Done" is read then it means that the particular menu is done and now a new menu needs to be made
...
void PrintMenu(){
    ...
    while (front->Name != "") { // continues to print the menu till the menus end
        cout << front->Name << " Menu:"<<endl; // print the name of the menu
        for (int i, j= 0; i < 100; i++,j++) { // for loop to go through the arrays
            if (front->Item[i] == "") { // if there is a new item in the array sinc the array contains 100 cells, i dont want to keep printing the "" cells
                // once the Items and prices are done printing resets the arrays to 0 for both arrays
                i = 0;
                j=0;
                break;} // since it is done with the items in the array then it breaks to the next menu if there is another
            else {
                cout << front->Item[i]<< endl; // since there are more items in the menu, this prints out the name of the menu item
            }
            cout << front->Price[j]<< endl; // prints out the price to the menu item
        }
        if (front->next != nullptr) { // if there are more menus then move to the next menu since the previous menu is done with items
            front = front->next; // points pointer front to the new menu and continues the process
        }
        else {break;} // if there are no more menus then break and the printing menu function is now finished.
    }
}
